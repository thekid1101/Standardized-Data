...
router.get('/', async (req, res) => {
  try {
    ...
    const topPerformers = await prisma.playerGameStat.findMany({
      where: {
        game: {
...
router.get('/top/:position/:season/:stat', async (req, res) => {
  try {
    ...
    const rawResult = await prisma.$queryRawUnsafe(statQuery, position, parseInt(season, 10));
    
    // Convert any BigInt values to regular numbers to avoid serialization issues
    const result = (rawResult as any[]).map((item: any) => ({
...
router.get('/compare/:position/:role1/:role2/:season', async (req, res) => {
  try {
    ...
    const comparison = {
      ...
      role1Stats: {
        targets: playsWithBothRoles.filter(p => 
          p.playPlayers.some(pp => pp.role === role1 && pp.targeted)
        ).length,
        completions: playsWithBothRoles.filter(p => 
          p.playPlayers.some(pp => pp.role === role1 && pp.completed)
        ).length,
        totalYards: playsWithBothRoles.reduce((sum, p) => {
          const playerPlay = p.playPlayers.find(pp => pp.role === role1);
          return sum + (playerPlay?.yardsGained || 0);
        }, 0)
      },
      role2Stats: {
        targets: playsWithBothRoles.filter(p => 
          p.playPlayers.some(pp => pp.role === role2 && pp.targeted)
        ).length,
        completions: playsWithBothRoles.filter(p => 
          p.playPlayers.some(pp => pp.role === role2 && pp.completed)
        ).length,
        totalYards: playsWithBothRoles.reduce((sum, p) => {
...
router.get('/trend/:playerId/:season/:stat', async (req, res) => {
  try {
...
router.get('/tendencies/team/:teamCode/:season', async (req, res) => {
  try {
...
router.get('/correlation/team/:position/:season', async (req, res) => {
  try {
    ...
    const correlationData = games.map(game => {
      ...
      const homePositionYards = homePlayerStats.reduce((sum, stat) => {
        if (position === 'QB') return sum + (stat.passYards || 0);
        if (position === 'RB') return sum + (stat.rushYards || 0);
        if (['WR', 'TE'].includes(position)) return sum + (stat.receivingYards || 0);
        return sum;
      }, 0);
      
      const homePositionTDs = homePlayerStats.reduce((sum, stat) => {
        if (position === 'QB') return sum + (stat.passTDs || 0);
        if (position === 'RB') return sum + (stat.rushTDs || 0);
        if (['WR', 'TE'].includes(position)) return sum + (stat.receivingTDs || 0);
        return sum;
      }, 0);
    ...
    correlationData.forEach(d => {
      ...
      if (d.won) teamData[d.team].wins++;
      else teamData[d.team].losses++;
    });
...
router.get('/situational/:playerId/:season', async (req, res) => {
  try {
    ...
    const playerPlays = await prisma.playPlayer.findMany({
      where: {
        playerId,
        play: {
          game: {
