...
router.get('/:code/roster/:season/:week', async (req, res) => {
  try {
    const { code, season, week } = req.params;
    const roster = await prisma.weeklyRoster.findMany({
      where: {
        team: code,
...
router.get('/:code/current-roster', async (req, res) => {
  try {
    const { code } = req.params;
    
    // Find the most recent week of 2024 season for this team
    const mostRecentRoster = await prisma.weeklyRoster.findFirst({
      where: {
        team: code,
        season: 2024
      },
    ...
    if (!mostRecentRoster) {
      ...
      const fallbackRoster = await prisma.weeklyRoster.findFirst({
        where: {
          team: code
        },
        orderBy: [
...
router.get('/:code/stats/:season', async (req, res) => {
  try {
...
router.get('/:code/position-analysis/:season', async (req, res) => {
  try {
    const { code, season } = req.params;
